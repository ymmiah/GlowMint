...
const StyleRemixModal: React.FC<StyleRemixModalProps> = ({ image, onClose, onApply }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[--color-bg]/80 flex flex-col justify-center items-center z-50 backdrop-blur-lg animate-fade-in p-4" onClick={onClose}>
      <div className="bg-[--color-surface-1] border border-[--color-border] rounded-lg shadow-2xl w-full max-w-3xl h-full max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-[--color-border]">
          <h2 className="text-2xl font-bold text-[--color-text-primary] flex items-center gap-3">🎭 Artistic Remix</h2>
        ...
        <div className="flex-grow p-6 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="flex flex-col items-center gap-3">
              <h3 className="font-semibold text-lg text-[--color-text-secondary]">Content Image</h3>
              <div className="w-full aspect-square bg-[--color-surface-inset] rounded-lg overflow-hidden flex items-center justify-center">
                  <img src={image.url} alt="Content" className="max-w-full max-h-full object-contain" />
              </div>
              <p className="text-sm text-[--color-text-tertiary] text-center">This is the image that will be transformed.</p>
            </div>

            {/* Style Image */}
            <div className="flex flex-col items-center gap-3">
              <h3 className="font-semibold text-lg text-[--color-text-secondary]">Style Image</h3>
              <div className="w-full aspect-square">
                ...
                <label
                  ...
                  className={`w-full h-full border-2 border-dashed rounded-lg flex flex-col justify-center items-center cursor-pointer transition-colors duration-300
                    ${isDragging ? 'border-[--color-primary] bg-[--color-surface-2]/50' : 'border-[--color-border] hover:border-[--color-primary] hover:bg-[--color-surface-2]/50'}
                  `}
                  onDrop={handleDrop}
                ...
                >
                  {isUploading ? (
                     <div className="text-center text-[--color-text-tertiary]">Loading...</div>
                  ) : styleImage ? (
                      <img src={styleImage.url} alt="Style" className="max-w-full max-h-full object-contain" />
                  ) : (
                    <div className="text-center text-[--color-text-tertiary] p-4">
               ...
               <p className="text-sm text-[--color-text-tertiary] text-center">Upload an image whose style you want to copy.</p>
            </div>
          </div>
          <div className="mt-6">
              <label htmlFor="remix-prompt-input" className="block text-sm font-medium text-[--color-text-secondary] mb-2">
              ...
              />
              <p className="text-xs text-[--color-text-placeholder] mt-2">Leave blank to let the AI figure out the style automatically.</p>
          </div>
        </div>
        ...
        <footer className="flex-shrink-0 flex justify-end items-center gap-4 p-4 border-t border-[--color-border] bg-[--color-surface-1]/50">
          ...
          <button 
            ...
            className="py-2 px-5 bg-[--color-primary] hover:bg-[--color-primary-hover] text-[--color-primary-text] font-bold rounded-lg disabled:bg-[--color-primary]/50 disabled:cursor-not-allowed"
          >
            Apply Remix
          </button>
        </footer>
